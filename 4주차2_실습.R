...
library(psych)
pairs.panels(air2[,4:9])  ##coValue(미세먼지), no2Value 비례관계/ pm10Value, pm25Value가 비례관계
...
library(ggplot2)
#정확성--그래프로 확인(*10 빼야함)
plot(formula=no2Value~coValue,data=train, col=alpha(c("purple","blue","green"),0.7)[train$cityName],pch=20)
points(formula=no2Value~coValue,data=test, col=alpha(c("purple","blue","green"),0.7)[test$cityName],pch=5)
...
##covalue, no2Value 비례관계/ pm10Value, pm25Value가 비례관계
...
air3[,2:3]<-scale(air3[,2:3])
summary(air3)

set.seed(123)

air_kmeans<-kmeans(air3[,-1],centers=3,iter.max=10000)
air_kmeans
...
##군집 확인
class(air3) ##ggplot은 dataframe 가능
class(air_kmeans$cluster)
air3$train_cluster<-as.factor(air_kmeans$cluster)

#그래프
ggplot(air3) + geom_point(aes(x=no2Value, y=coValue,col=train_cluster,group=train_cluster))
       
#수치                                              
table(air3$cityName, air3$train_cluster) ##오차가 있음을 확인 할 수 있음(완벽하지 않은 모형)
...
k_max <- 15
wss <- sapply(1:k_max, 
              function(k){kmeans(air3[,-1], k, iter.max = 1000 )$tot.withinss})
